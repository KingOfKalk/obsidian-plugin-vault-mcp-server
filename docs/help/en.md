...
- **`daily-note`** — optional argument: `date` (ISO `YYYY-MM-DD`; omit for today). Asks Claude to call `vault_daily_note` (which resolves the daily note's path and creates it from the configured template if missing) and then `workspace_open_file` so the note becomes the active leaf.
