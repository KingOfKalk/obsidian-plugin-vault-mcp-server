...
| Broken-template fallback              | Create with empty body if the configured template path can't be read. (No warn-log in v1 — vault handlers do not yet take a logger; threading one through is out of scope for #304.) | The user has explicitly asked for today's daily note; a missing/moved template shouldn't block that. A warning surfaces the misconfiguration without failing the call.                                                 |
...
2. Resolve the target moment using `import moment from 'moment'` (the direct npm dep — Obsidian's `.d.ts` types `moment` as a non-callable `typeof Moment` namespace, so it can't be invoked when imported from `'obsidian'`):
